  ...
  module Assets
    RSpec.describe DocumentScanner do
      ...
      describe 'malformed input terminates' do
        ...
        it 'survives 500 references without quadratic behaviour' do
          html = (1..500).map { |n| %(<img src="/a#{n}.png">) }.join
          started = Process.clock_gettime(Process::CLOCK_MONOTONIC)
          refs = scan(html)
          elapsed = Process.clock_gettime(Process::CLOCK_MONOTONIC) - started

          expect(refs.length).to eq(500)
          # Generous, because a shared runner is noisy. What it catches is the
          # `@html.downcase` per element that the first draft had, which is quadratic.
