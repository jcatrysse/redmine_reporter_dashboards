...
| T-38 | **DONE — one print-first stylesheet, inlined into both bindings, and two capability claims retracted the same day they were written.** `report_stylesheet.rb` (the CSS, built in Ruby so the palette and three of the five type steps are READ from `Charts::Palette`/`ChartLayout` rather than restated — a `.css` file cannot read a constant, which is the whole reason the file is Ruby), `report_document.rb` (the ONE assembler; `ReportFrame.document` delegates and keeps the CSP, `ReportRun#pdf_document` wraps the PDF body, which used to reach the engine as a bare FRAGMENT with no head to put a stylesheet in), `script/gates/chrome_no_design_tokens.sh` (§9b's "no design language of its own for chrome", enforced: no colour, typeface, font size, line height, custom property or `@font-face` under `assets/stylesheets/` — 12 planted violations caught MID-FILE, class names and comments not findings, an absent subject a hard failure), three conformance fixtures (**F-21** thead across a 6-page table, **F-22** 14 blocks at drifting offsets, **F-23** drill-through `/URI` annotations), a `links` probe that CROSS-CHECKS its own byte scan against poppler so a harness blind spot cannot be reported as an engine defect, `{% mermaid title:/desc: %}` plus the boot-script insertion that gives a diagram a `<title>`/`<desc>` (F-17: Mermaid draws in the BROWSER, so this is the only place it can happen), and two new linter rules for FR-76's colour-alone clause. **THE CHROME STYLESHEET'S MERMAID RULES WERE DEAD CSS** — they styled the report body from Redmine's page head, and every surface that shows a body puts it in an opaque-origin `srcdoc` iframe, so FR-68's "emitted, labelled, never a blank space" was emitted-and-unlabelled since T-35. **Two capabilities (`:repeating_table_header`, `:svg_link_annotations`) were added to the CLOSED vocabulary on a measurement and RETRACTED on a better one**: the first was taken against `apt install wkhtmltopdf` (unpatched Qt), which answers no to both; the patched build CI uses answers yes to both, so no support difference exists. HANDOVER §1 carries that trap in capitals and it still cost a round — `Render::Capabilities` now carries the retraction and the rule ("a capability is a claim about every SUPPORTED build"). Corpus after the fix: **chromium_cdp 23/0/0, gotenberg 22/0/1, wkhtmltopdf (patched qt) 21/0/2 — 100 examples, 0 failures**, matrix regenerated. Three defects its own tests found: the gate reported OK while its comment-stripper crashed (an empty pipe is indistinguishable from a clean file), the boot script's label insertion put the `<desc>` in FRONT of a `<title>` Mermaid had emitted itself and destroyed the accessible name, and the lint rule's `legend:` value test matched `legend: true` because an optional `\s*` in front of a negative lookahead makes the lookahead optional too. **Its independent review REJECTED it with five blockers, and two of them were the same defect one level deeper than the fix.** (1) The gate STILL reported OK while reading nothing: the strip check had been moved out of the pipeline but stayed inside the function the arms call through `$( … )`, and an `exit 2` in a command substitution exits a subshell — so CI's step was green. It is now checked once at top level, and the gate has a committed `_selftest.sh` (33 cases) that drives both self-protection arms by BREAKING THE READER FOR REAL (an invalid UTF-8 byte; an all-comment stylesheet) rather than by patching an arm; ten mutations of the gate, ten caught. (2) `PdfProbe.links` raised a spurious HARNESS failure for any URL containing `&`, because `pdftohtml -xml` XML-escapes its attribute and the byte scan does not — and F-23's URLs were single-parameter, so the corpus was green while a real `&`-dense drill-through URL would have detonated the guard and blamed the reader. `CGI.unescapeHTML`, plus `spec/conformance/link_probe_spec.rb` (9 examples over hand-built PDFs), which the whole reader had lacked. (3) The lint rules were case-sensitive while `bool_param` and `resolve_type` both downcase — `legend: TRUE` was a false `:error` that forced `rework?`, and `type: PIE` MISSED the one defect the clause exists for. (4) `desc:` was unbounded, the cap having been applied to `title` only. (5) The Ruby half of the mermaid clause had no test at all; it now has 25, and two of them exist because a mutation survived. Seven majors besides: `box` on an `<iframe>` inside a view that already had one (and content-box arithmetic overflowing its container by 22px — it goes on a container now), a stale measured claim in `pdf_inspector.rb`, two README overclaims about wkhtmltopdf, a "SAME OBJECT" identity claim no `include` matcher could hold (replaced by two source-level examples asserting one assembler and one `<style>` emitter, with a four-entry exemption list for the engine preflight probes and a staleness check on it), an unmeasured "a row is never split" promise, and a second exception class for one condition in the probe's tool check. All fixed. Mutations: 45 across the session (5 stylesheet, 6 document/frame/run, 4 linter, 5 boot, 10 tag, 10 gate, 5 misc), 43 killed, 2 survivors closed with the examples they were missing |
| T-37 | **DONE — the authoring experience: a lint panel with line and column, a drop reference generated from the code, a starter gallery that renders on every engine, and a chart form that writes the tag.** Four increments. `template_linter.rb` (Finding gains `column`, and `#position` is the one spelling all three surfaces print), `lint_report.rb` + `rake reporter_dashboards:lint_templates` — **the task §9b.1 has named since it was written and which had never existed**, so the parity it promised had nothing to be parallel to — `_lint.html.erb` (server-rendered, bounded at 100 rows with the drop STATED), `_editor.html.erb` (ONE editor on three pages; `preview` now hosts it, which is what finally satisfies §9b.2's *"diagnostics in the editor, next to the code"* — before this they were on a page with no code and the way back was the browser's Back button), `liquid/drop_reference.rb` + `docs/drop-reference.md` + `script/gates/drop_reference_parity.sh` (FR-72, both directions, four arms), `starter_gallery.rb` + five `starters/*.liquid` + `reporting/gallery_check.rb` + `rake …:gallery:verify` + committed thumbnails with the digest of the body each was drawn from, and `_chart_form.html.erb` + `chart_form.js`. **`[OQ-M]` IS WHY THE PANEL IS NOT A GUTTER** and the `Accept:` line was reworded before a line was written. **MEASURED: five starters x three engines = 15 of 15** (chromium_cdp Chrome/141, gotenberg 8.35.0, wkhtmltopdf 0.12.6.1 patched-qt), and every starter lints at **zero** rather than at a ratchet. **THE QUADRATIC LINTER IS FIXED, and that was the reported hold-a-worker defect this whole surface would have been built on**: character indexing into a multi-byte String is O(index), so `TemplateLinter.analyse` took **35.6 s** at its own 512 KiB bound — 0.002 s now, the real 33 KB example 0.277 s → 0.028 s, a tag-dense multi-byte body at the bound 39.7 s → 0.71 s. Three contained changes: the body is scanned as BYTES (every pattern is ASCII-only, asserted, and the column and excerpt are decoded back), `HtmlScanner` jumps to the next `[<{]` instead of stepping per character, and the two region lists are computed once per analysis instead of about fifty times. **Its own harness found three defects in the starters and one in the tag** — the spent-time starter asked for `from:`, `measure: sum` and `of: hours`, none of which that source has; and **`group_by: user` cannot be expressed on the spent-time source AT ALL**, because a bare tag parameter is resolved as a Liquid variable, `user` is always assigned, and QUOTING DOES NOT HELP (`parse_markup` strips the quotes before `str_param` looks the value up). That last one is **reported, not fixed**: 23 call sites go through that method and making a quoted parameter a literal changes the meaning of every quoted parameter in every existing template. **Two mutation survivors, both closed**, and the second is the entry worth keeping: the thumbnail staleness detector had four tests and every one of them passed against a version that always answered `[]`, because none had ever shown it a changed body. **Two deviations from §9b.1, both reported rather than absorbed**: the two legacy examples are NOT gallery entries (12 and 44 lint findings each, all of them the hand-built Chart.js 2 plumbing `{% chart %}` replaced — a rewrite of an 800-line dashboard, not a clean-up, and they keep a second job as the README's record of the old idiom), and the chart form has NO "drill on/off" because `{% chart %}` has no such parameter — drill-through comes from the aggregation, and a control writing `drill: true` into a chart tag would emit a parameter the tag ignores, which is the "stores state that is not real" failure the clause exists to prevent. **CI HAS BEEN READ FOR THIS TASK, WHICH IS NOT USUAL HERE.** Run 250 (`6bc7d42`) is **26 of 27 jobs green**: all four `RSpec` jobs recovered — they had been RED since T-38 over a source-level glob that rejected any path containing `/redmine/`, i.e. every path once the plugin is installed the way CI installs it, and one of that spec's three examples had been passing VACUOUSLY because an empty glob satisfies an emptiness assertion. The one red cell is the NEW `starter-gallery` job at **14 of 15**, and it is about the runner rather than the starters: `issue-document / chromium_cdp` timed out as the FIRST render of the job while the other four chromium renders passed. Every engine is warmed through `render:preflight` now — and the Gotenberg warm-up that step used to do was posting the wrong form field and had never warmed anything. The plan's `Touches:` line naming `render/drop_reference.rb` is **impossible**: `layer_purity.sh`'s render arm forbids `render/**` from naming `Liquid`, so the reference lives in `liquid/` |
| T-27 | **DONE — the upgrade diagnostic, the gate that was only a comment, and the decision the task owed.** `permissions/authoring_audit.rb` (pure, duck-typed on four readers so every rule is asserted DB-less), a section on the preflight page rendered on the GET, 11 locale keys × 9, `script/gates/no_html_safe.sh` + `_selftest.sh` + allowlist wired into CI's `gates` job, and a README section. **THE DECISION INVERTED THE BRIEF'S GUESS.** The brief said `init.rb`'s boot line was "the upgrade diagnostic in embryo"; it must not be, because a permission grant is a string in `roles.permissions` and NOTHING reconciles that column against the plugin registry — so a role keeps the base plugin's authoring permission after that plugin is uninstalled, invisible on the roles screen (which renders `setable_permissions`), and a diagnostic gated on `ReporterPresence` would print an empty list in exactly the case it exists for. The audit therefore reads Redmine's permission tables and asks the registry nothing; the detection keeps its ONE real consumer, gating the `REPORTER_GLUE_FILES` require. **The base plugin's permission was MEASURED, not inferred** (its repo at `b1d1736`): `init.rb:19-32` registers six and exactly one is code execution — `manage_report_templates`, whose `#create` is `params[:report_template][:type].constantize.new` plus safe_attributes from params; the other five render STORED templates, and listing them would cry wolf on every role that may read a report. **The asymmetry the page exists to show**: that permission carries no `require:`, so `setable_permissions` subtracts nothing for Non-member or Anonymous and an administrator can tick code execution onto Anonymous, while ours cannot land there because `Entry#requires` derives `:member` from `authoring` — hence `Role.all` rather than `Role.givable`, which would filter out the worst row. **Its independent review REJECTED it with two blockers and four criticals.** The gate was wired into NOTHING (`technical-spec.md:1581` names it in the `gates` job), so the one mechanical thing the `Accept:` line demands was still enforced by nobody; and it had four holes — `exit 2` inside a command substitution's subshell so a crashing search printed PASS and exited 0 (the trap `ci.yml` documents forty lines away, hit a third time), `find … 2>/dev/null` making a gate certify INV-9 having read no files, `<%==` and `<%= raw ` unmatched though the former is exactly the latter, and an empty-buffer exemption that dropped the whole LINE so `''.html_safe << user_input.html_safe` passed. The committed self-test then caught two more of the author's own within a minute: `sed` without `-E` made the exemption's metacharacters literal, and the widened `raw` pattern matched `raw = "…"`, an ordinary local variable, reporting 20 real files. **A claim was also overstated INTO THE README**: "`Role#allowed_to?` keeps honouring the grant" is true at that method (`role.rb:304-311` applies no registry filter) but every project-scoped check returns false first at `Project#allows_to?`, and the base plugin declares the permission inside a `project_module` — so while it is uninstalled the grant authorises nothing and re-arms on reinstall. Corrected in four places; the diagnostic never needed the stronger claim. `permission_map_spec.rb:673` was RED on the first commit because it greps whole files for `add_permission` and the new module's comment cites core's `add_permission!` — E-14 again, prose counted as code; it strips full-line comments now, negative-tested with a real planted grant. Deviation from §4's single-budget gate wording recorded in `technical-spec.md`, per G9. **`ZERO_REPORTER_MODE=strict` did not move (6 files)** — the diagnostic names the base plugin nowhere, because `zero_reporter.sh` searches `config` and nine locale files naming it would have added nine entries to the list T-27 exists to shrink |
| T-26 remainder (S-30) | **DONE 2026-08-13 — `glue/` is deleted, and this time it was a deletion.** The curator authorised it on 2026-08-12; that attempt measured 166 of 249 DB-less examples red and was reverted, because the tag suite used the legacy path as its HARNESS rather than merely testing it. So the harness was rebuilt FIRST: `spec/sql_aggregation` now builds every context from an owned `RenderContext` (`owned_registers`/`owned_query_registers`) and passed identically with the module present and with `legacy_available?` stubbed false — 695 examples, 0 failures both ways — before a single file was removed. Deleted: `glue/` entirely (`scope_resolution.rb` and `reporter_list_patch.rb`, a producer/consumer pair joined by a thread-local), `spec/reporter_list_patch_spec.rb`, `test/unit/golden_scope_fixture_test.rb` (16 test methods — the plan said twelve, counted rather than trusted), `ScopeBinding#legacy_bind`/`#legacy_host`/`#legacy_available?`/`LegacyHost`, `REPORTER_GLUE_FILES` and the `reporter_present?` branch in `load_patches`, and the `ReporterListPatch` line in `apply_reporter_patches`. **38 examples went and every one had the resolution as its SUBJECT**, not as its harness; the argued list is in `liquid_aggregate_tag_spec.rb` where the blocks used to be. **NOT A VISIBILITY REGRESSION, and that is the load-bearing claim**: `enforce_visibility` existed because a legacy source could hand over an arbitrary relation, and an owned context cannot — the constructor REFUSES a nil actor and the scope comes from `ReportScope.build` over that actor's visible scope, so the intersection moved upstream and became unconditional (`multi_actor_visibility_test.rb` asserts it against a real `Role#issues_visibility`). One assertion CHANGED rather than moved: the `query_id` lookup expected `User.current` and now expects the context's actor, because the owned path passes it — keeping the old value would have been asserting the defect. **`spec/golden/scope/scope.jsonl` and `sql/scope_sql.jsonl` survive byte-identical** as the record of what the six sources answered (`git diff` over `spec/golden/` shows only README changes); `spec/golden/README.md` says they are historical and must not be tidied away. `no_thread_local.sh`'s exemption list is now EMPTY — both entries were `glue/` — so it asserts no per-thread state anywhere under `app/` or `lib/`, and it was hardened for the empty-array-under-`set -u` case and re-negative-tested. `ZERO_REPORTER_MODE=strict` goes **6 → 5**. **The detection SURVIVES for a smaller reason than T-27 recorded**: its consumer is no longer the glue require but `apply_reporter_patches`, which now installs exactly one thing — `reporter_report_content_patch`, a performance fix on the host plugin's own report generation. Whether that ships at all is a curator decision. Self-review found two defects before the independent one: `SPEC_ACTOR` assigned inside `RSpec.describe` defined **`Object::SPEC_ACTOR`** and gave both tag specs one shared actor (Ruby warned; both suites passed anyway because the Structs were equivalent — a collision that passes), and four comments still described the legacy path as live |
| Curator decisions #2-#8 | **DONE 2026-08-13/14 — seven of the eight settled items are implemented; only #1 is left.** `docs/plan/DECISIONS-PENDING.md` carries the table and, at its foot, the SCOPE of what #1 still owes. **#2** closed defect D-1's second half — `measure_groups` dispatches to `grouped_measure`, which plucks the grouped aggregate POSITIONALLY, the same shape `grouped_counts` has used since T-08; still one G7 hunk and `RATCHET` still 1, because the new methods sit inside the region the `grouped-counts` entry already licenses. **MariaDB itself is CI's to answer** — it is not installable beside the MySQL client in this container. **#3** is the breaking one and it was NOT what the report said: the defect was in the PARSER, not in `str_param`. `PARAM_RE` captures a double-quoted, a single-quoted and a bare value into three groups and all FIVE `parse_markup` copies collapsed them into one String, so quoting no longer existed by the time a value was read. One owned module (`liquid/tag_params.rb`) now says the rule once — quoted is literal, bare is a variable falling back to the literal — and five tags share it. **Five copies, not four**: `mermaid_tag.rb` carried a fifth, which the decision's own count missed. **Four readers, not one**: `int_param` does its own lookup and `ScopeBinding#query_id_of` is a fifth copy in another file. **Twelve mutations, five survivors, all five closed**, and the sixth is recorded as EQUIVALENT with the difference CONSTRUCTED rather than read for (two `ChartSpec`s built with `title: nil` and `title: ''` are `to_h`-equal, because `#initialize` runs both through `presence`). **#4** narrows the authoring diagnostic to the permissions that genuinely write a template body — derived from the flag minus a NAMED exclusion, so a fifth authoring permission is reported by default; the method is public because the claim is unobservable through `rows`. **#5** marks the two comment-only files `[permanent]`, taking `ZERO_REPORTER_MODE=strict` **5 -> 3**. **#6** corrects the README's spent-time table from eleven dimensions to the real eight. **#7** emits `<html lang>` from `Setting.default_language` and emphatically not `I18n.locale`; a mutation then showed the attribute ESCAPE was untested and the comment beside it wrong about which input it guards — `LANGUAGE_TAG` guards the setting, `escape_attribute` guards the public `lang:` seam. **#8** renumbers FR-50 to FR-27/FR-74 in nine locale headers. **#1 IS DELIBERATELY NOT STARTED**: it is the only item with an irreversible deletion, its safety rests on proving no context-less render remains, and S-30 is this project's own evidence that a deletion half-done is worse than one not begun |
...
| **T-44** | **done** — `ReportFrame::AUTO_HEIGHT_SCRIPT` inside the document, the parent half in `redmine_reporter_dashboards.js`, and the partial's header comment rewritten so the file no longer argues against its own code. **A ONE-WAY POST WAS NOT ENOUGH AND THE FIRST VERSION LOST HALF ITS MESSAGES**: a frame can finish before the parent listens, and on `/my/page` it always does, because the parent script is included in BODY position there — a my-page block renders long after the head is emitted. So the listener installs at parse time and the parent ASKS every frame on ready and on load; the frame answers. Measured in a real browser on three surfaces, three runs, identical: dashboard widget 244px fixed -> 536px measured, template page -> 1483px, my-page widget -> 668px. 9 node-driven examples cover the adversarial half a browser will not send. §Findings **M-5** |
...
> **CLOSED 2026-08-13.** The diagnosis below was right and the prescription was followed
> exactly: the harness was rebuilt first, the suite was proved indifferent to the module
> (695 examples, 0 failures with it present AND with `legacy_available?` stubbed false),
> and only then was anything deleted. See the §Status row for what went and what survived.
>
> **The number to carry forward is not 166 — it is that a suite can depend on the thing it
> appears to test.** The 150 failures were not testing `ScopeResolution`; 121 of them were
> testing the TAG and merely using a legacy source to hand it a scope, which is why four
> harness-helper edits fixed most of them and only 38 examples were genuinely about the
> deleted behaviour. The first attempt read "166 red" as "166 examples to port" and
> reasonably concluded it was a task; the split is what makes it a day rather than a week.
> **Measure the split before estimating a rebuild.**
...
| **THE FR-50 CODE REVIEW COULD NOT REJECT IT, AND FOUND SEVEN MUTANTS THE AUTHOR'S HARNESS HAD NOT.** 31 mutations of the reviewer's own choosing, 24 killed: every step of the precedence chain is pinned by a discriminating example, including the two that would take a production install down (`registry.fetch` on a stale selection, and a hint losing to the setting). Of the seven survivors, three are genuinely equivalent, one is minitest-only by design (the `FROM_SETTINGS` sentinel), and **three were real gaps**: `ASSET_CAPABILITIES = %i[]` and `dropped? → false` survived because the expectation read the constant under test on both sides, and the blank guard in `selected_engine_id` was covered by nothing in the tree | the reviewer's runs, each escalated to the full 2663-example suite | yes — the three literals are written out, `dropped?` is asserted in the true direction, the blank guard is asserted through its only observable (the log line an operator would read as an install that selected an engine called nothing), and the limit is tested AT 64 and one past it. Plus the rake task's port, which the review found asserted by nothing in the file whose stated purpose is that exact class of glue failure — now asserted on the constructor in both directions, and dropping it is red |
...
| 2 | ~~**A name that does not resolve and a socket that refuses get one sentence and one code.**~~ **CLOSED 2026-08-11 — the arm was split, the CODE was not, and the first attempt to split both was rejected (§Findings E-30).** `SocketError` (which `Socket::ResolutionError` subclasses, so one rescue covers Ruby 2.7 through 3.4) gets its own sentence, naming the endpoint, `RRD_GOTENBERG_URL`, the Docker network **and the transient case**; the code stays `:engine_unavailable`, because `SocketError` is every `getaddrinfo` failure including EAI_AGAIN — an unreachable resolver, which a retry fixes and which `:engine_misconfigured` promises can never be fixed by one — and because `technical-spec.md` §5 states normatively and ONCE that reachability is therefore `:engine_unavailable`. `Errno::ECONNREFUSED` is not a `SocketError`, which is what keeps the refused socket on the generic arm. A row in the thirteen-row table, an example asserting the two SENTENCES differ, a second asserting the arm catches the class production actually raises rather than only its parent, and **five mutations red** (the code moved back, the arm narrowed, the endpoint dropped from the sentence, and each of the two remedies dropped from it). The original finding, for the record: ** `Socket::ResolutionError` and `Errno::ECONNREFUSED` both produce *"nothing answered at …, Confirm the container is running and that Redmine can reach it at this address"* | both, against a real container and a dead port; the class survives only in `detail`, which the admin page prints in a column headed **Engine version** | **Recommend a named arm** for an unresolvable host — *"the name X does not resolve from Redmine; check the spelling of RRD_GOTENBERG_URL and that Redmine is on the same Docker network"* — which is the one of the three faults whose remedy is unambiguous. Two lines and one example, and it is a diagnostic-message change rather than a code change, so it does not belong in the commit that introduces a code |
| 13 | **`String#inspect` does not escape bidi or zero-width characters**, so a crafted `render_engine` value carrying U+202E reverses the rendering of its own list item on the settings page | measured. Requires an administrator to have crafted the POST (the field is a `<select>`), is confined to one `<li>`, and every character that matters is HTML-escaped | Pre-existing and CONSISTENT: the identical construct renders the asset, mail and engine dropped-value lists. **Recommend** fixing it once for all three (strip `\p{Cf}` and the bidi overrides before `.inspect`, or render the value in a `<bdi>`), not a third of it here |
| 14 | ~~**`asset_models: []` validates, and renders as `—`**~~ **CLOSED 2026-08-11** — `EngineCatalogue` now raises, in the same shape as the `default:`-on-exactly-one-engine check beside it, and the example plants the empty list in a copy of the real file. The original finding: ** — indistinguishable from an engine the catalogue never described, while its inability to carry ANY asset is excluded from "Not supported" | read: `EngineCatalogue#engine_asset_models` rejects unknown models, not an empty list. `Resolver` refuses every reference for such an engine, so it is a real absence rendered as a dash. Unreachable in the shipped file | **Recommend** one `raise` in `EngineCatalogue`: an engine must declare at least one asset model. It is the same class as the `default:`-on-exactly-one-engine validation that file already performs |
| 16 | **THE JAVASCRIPT PREFLIGHT CHECK FAILS AGAINST A HEALTHY, FRESHLY-STARTED CONTAINER, and its remediation is wrong when it does.** Found on 2026-08-11 by running `script/render_preflight_exit_codes.sh` against a container created two minutes earlier: **three consecutive runs red**, arm 1 (`exit 0` against the authenticated service) reporting *"this Gotenberg did not answer the JavaScript check in time"*. The container was fine | THE MECHANISM, read off Gotenberg's own access log rather than guessed. A plain conversion takes **0.15–0.38 s**. The JS probe — the only check that sends `failOnConsoleExceptions` — is the FIRST conversion the container ever sees, because `configuration_checks` run before the document checks, so it pays Chromium's cold start: `convert HTML to PDF: convert to PDF: process first start: context canceled`, **latency 10.011 s**, i.e. OUR `PROBE_TIMEOUT_MS` cancelled it while the browser was still starting. A retry then gets `start process: browser start already in progress`. Warm the browser with ANY conversion and the contract holds — measured, two consecutive clean runs. **CI is unaffected**: `render-smoke` runs the corpus first, so something else pays the cold start there, which is why run 31488625241 was green | This is the README's own instruction failing for a first-time operator: start the container, run the preflight, get told the container has JavaScript disabled when it does not — the confident-wrong-remediation shape, for the fourth time in this adapter's history. **Recommend fixing it, in its own change, and NOT by lengthening the probe** — a bigger number hides the same race behind a slower failure. Two candidates: warm deliberately (one cheap conversion before the configuration checks, and report ITS failure honestly), or treat Gotenberg's own `process first start` / `browser start already in progress` as a named RETRY-ONCE condition, which is a string this adapter would then depend on. Not done here because the JavaScript check is the most delicate in the adapter — E-27 measured three of its four arms unable to fail when first written — and a retry inside it belongs in a change whose subject it is (§11.5) |
| 15 | **The exit-code contract's SELECTION arm is covered in-process only.** `script/render_preflight_standalone.rb` has no Redmine and therefore no `Setting`, so FR-50's selected engine cannot reach `PreflightCommand` through the one driver that reads exit codes off a real process | read, and confirmed by an independent review | **Recommend** leaving it: the alternative is an `RRD_SELECTED_ENGINE` variable production does not have, which would make the script prove something the product does not do. The driver's header now says so out loud, and the rake task's port is asserted in `test/unit/render_preflight_rake_test.rb` |
...
| 3 | ~~**Two pre-existing `:engine_unavailable` sites fail the new discriminator.**~~ **CLOSED 2026-08-11 — one moved as recommended, the other turned out to be TWO states and only one of them moved (§Findings E-30).** `wkhtmltopdf.rb`'s `EACCES` arm is now `:engine_misconfigured` with its own sentence (*"permission to execute the render engine at the configured path was refused — check its execute bit, the directories above it, and whether that filesystem is mounted noexec"*, plus a separate sentence for a path naming a DIRECTORY, which is the one cause this side can establish. Three earlier drafts each named a cause and each was refuted by measurement — §Findings **E-31** row 3, **E-32** row 2 and **E-33**), and `ENOENT` keeps `:engine_unavailable` and the two-remedy sentence, because a missing path might be an uninstalled package or a mistyped `RRD_WKHTMLTOPDF_BINARY` and this side cannot tell. `ReportRun#no_engine_diagnostic` was moved wholesale and a review then measured the premise as false for half its traffic — it is now two states: *nothing registered* keeps `:engine_unavailable`, *registered, and every engine needs a service so none can be chosen automatically* is `:engine_misconfigured` and names the setting `technical-spec.md` §5.2 clause 4 put there. Seven spec expectations, **14 mutations red here and 20 more in the review that followed**, which found two survivors this set had no mutant for (E-31 row 1). **A note for the next reader: `File.chmod(0o000)` DOES produce `EACCES` on exec even as root** — `execve` refuses a file with no execute bit for anybody — but the example now uses `0o644`, because at `0o000` root can still READ the file and the two faults stay tangled; `0o644` isolates the missing exec bit and is the realistic mistake. The original finding: ** `wkhtmltopdf.rb`'s `Errno::EACCES` arm (a binary that exists and is not executable, from an operator-typed `RRD_WKHTMLTOPDF_BINARY`) and `ReportRun#no_engine_diagnostic` ("no render engine is registered") | read, and both are unambiguous under the new rule: an operator's action fixes them and a retry never will | **Recommend moving both**, in their own change with their own tests. Deliberately not done in the commit that adds the code: a vocabulary addition and a re-classification of two unrelated call sites are two reviews, not one, and `no_engine_diagnostic`'s move touches the exit-code surface (`PreflightCommand` already reports that state as `NOTHING_TO_RUN`) |

**E-30 · THE FOLLOW-UPS E-29 RECOMMENDED WERE IMPLEMENTED WIDER THAN THE RECOMMENDATION, AND A
FOURTH REVIEW REJECTED THEM FOR IT.** 2026-08-11, same day, `3cacd82` reviewed and corrected in
the commit after it. Two of the four moves were right; the two that were not were both cases of a
code change smuggled in beside a message change.

| # | What the review found, and what it cost | Evidence | State |
|---|---|---|---|
| 1 | **The DNS arm moved the CODE as well as the sentence, and E-29 row 2 had explicitly recommended only the sentence** (*"it is a diagnostic-message change rather than a code change, so it does not belong in the commit that introduces a code"*). Two independent refutations: `technical-spec.md` §5 states — once, deliberately, *"because two normative statements of one rule is how a vocabulary acquires two meanings"* — that reachability and transport are **therefore** `:engine_unavailable`, so the code change contradicted the contract while the spec went unedited; and `SocketError` is not "the name is wrong" but every `getaddrinfo` failure | the reviewer reproduced EAI_AGAIN end-to-end with no doubles, in a private mount namespace with an unreachable `nameserver` bind-mounted over `/etc/resolv.conf`: the address spelled correctly, the network right, and the operator told to *"Check the spelling"* — under a code whose own contract promises *"no retry will ever produce a different answer"*, when a retry is exactly what fixes it | **FIXED.** The arm and its sentence stay; the code is `:engine_unavailable`; the sentence now names BOTH remedies and the endpoint. `Socket::ResolutionError#error_code` would discriminate EAI_AGAIN, but only on Ruby 3.3+, so the split would hold on three of four CI cells and guess on the fourth |
| 2 | **`no_engine_diagnostic` is two states wearing one sentence, and the moved sentence is FALSE in the state its own tests cover.** *(This row said "Both mutants are now killed". A fifth review measured that as HALF TRUE and the claim is corrected in §Findings **E-31**: the MESSAGE mutant is killed, the DETAIL mutant survived — the old `'Render::Registry.ids is empty'` string moved to the empty-registry arm where nothing asserted it. `63f080e`'s commit message carries the same overclaim and cannot be edited; this is the correction of record.)* The comment argued *"`Registry.ids` is empty, nothing is ambiguous"*; `resolve_engine` also answers nil when engines ARE registered and none is auto-selectable, which is what two of the three moved expectations set up — each asserting `Registry.ids == [:gotenberg]` three lines above | the reviewer probed the shipped object through that very example: `code=:engine_misconfigured`, `message="no render engine is registered…"`, `detail="Render::Registry.ids is empty"`, with gotenberg registered. Printed by the diagnostics panel and the failure mail — **not** the failure PDF, which E-31 row 2 measured as carrying a deliberately narrower field set. **Mutating that message to nonsense left the FULL suite green — 2670 examples, 0 failures** — and so did mutating the detail | **FIXED, and this one predates the change it was found under.** Two states, two codes, two true sentences; the misconfigured one names the Plugins page under Administration. The message mutant is killed here; the DETAIL mutant needed one more round (E-31 row 1) |
| 3 | **The supporting analogy was inverted.** The comment claimed `PreflightCommand` "already reports the same state as `NOTHING_TO_RUN`" | measured: an EMPTY registry exits 2, but *only a service engine, none selected* exits **0** and prints *"OK so far"* — `nothing_registered` fires on `reports.empty?` alone | **FIXED by deletion.** The comment now names the disagreement instead of claiming agreement, and points at **E-27** row 7 — this row cited E-29, which has its own unrelated row 7 — and it is **CLOSED 2026-08-12**, the preflight having been the wrong one; it now exits 2 |
| 4 | **`failure.rb` stated the split as a property of the vocabulary while only one surface implemented it**, and the render path is pinned the other way by `gotenberg_spec.rb:1025` | same fault, two codes, measured on both surfaces | **DISSOLVED by fixing #1.** With the code reverted, both surfaces answer `:engine_unavailable` and only the sentences differ, which is what the spec allows |
| 5 | **Bookkeeping that two files disagreed on.** `failure.rb` said "seven arms" where the plan said six; a "twelve-row" table had thirteen rows in three places; `failure.rb` listed `no_engine_diagnostic` as a producer of a `Render::Failure` code when it mints a `Reporting::Diagnostic` | `rg -c 'code: :engine_misconfigured'` | **FIXED.** Six is right again after #1, and the `Diagnostic` distinction is now stated where it is read |
| 6 | **`File.chmod(0o000)` is the weaker fixture** — as root `File.readable?` is true at `0o000`, so it leaves "cannot read" and "cannot execute" tangled | measured both modes through `popen3` | **FIXED**, `0o644`, which is also the realistic mistake |
| 7 | **A `def` inside an RSpec example lands on the example-group class.** I flagged this myself as a HANDOVER §1 neighbour; the review **refuted the danger by measurement** — a sibling example does not see it, because RSpec runs each on its own instance | the reviewer's own probe | **CHANGED ANYWAY**, to a lambda: harmless is not the same as worth a reader's double-take |
| 8 | **The `SocketError` arm was only ever tested against the parent class.** Production raises `Socket::ResolutionError` | measured on 3.3.6, `error_code` −2 | **FIXED**: a second example raises the real class where it is defined and skips with a reason where it is not — two of CI's four cells run 3.3+ |

**What the round is worth as evidence:** 14 mutants written against the fixes, **14 killed, 0
survived**, control green first and every verdict parsed off the real summary line. **That is 14 of
MY mutants; the next review wrote 20 and found 2 survivors my set had no mutant for** (§Findings
**E-31**), which is the honest measure of what a self-written mutation set is worth: it covers what
its author thought to attack. In particular the claim that "the two mutants that survived the
previous round are both killed now" was half true — the message, yes; the detail, no.

**And one trap that makes several earlier numbers in this file mean less than they look.** Running
the suite from a `git archive` extract — which is how a clean baseline gets built, and how several
measurements in this session were taken — **silently skips the ten `spec/golden` byte-identity
examples**, because they resolve the `v0.5.0` baseline through `git`. That is gate **G7**. A run from a copy and a
run from the real tree report the SAME example total and differ by exactly ten PENDING — measured
on one tree at `cdb272e`: working directory `2673, 0 failures, 127 pending`, `git archive` extract
`2673, 0 failures, 137 pending`. Only the second is evidence for G7. (An earlier version of this
paragraph compared `2670/137` with `2671/127` and framed the example counts as differing too; they
were one apart for an unrelated reason — the round had added an example — and a review caught the
conflation.) Measured directly: `spec/golden/baseline_spec.rb` → 7 examples, 0 failures in the
working tree; the whole of `spec/golden` → **167**, 0 failures. (This said 71, which was a file
...
mutations and could not make a legitimate all-deferred run exit anything but 2, nor make a real
failure hide behind it, and it confirmed all three of the claims that round set out to correct.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **The correction to the *"OK so far"* claim over-reached into a new false one**: it said the line is *"shared with the admin page"* and that rewording it would be a change *"on two surfaces"* | measured twice. Statically: `headline`'s only caller is `Report#to_text`, whose only production caller is `#emit`. By mutation: `headline` replaced with a constant left the admin page's functional tests **green** (25 runs, 0 failures) while the render specs went red — and the rendered page for the same install prints *"Nothing failed, but 1 check(s) could not run"*, from `ReporterPreflightHelper#reporter_preflight_summary`, with its own nine locale keys | **FIXED** in the comment and in E-37 row 2. The DECISION does not change — rewording `headline` is still not done — but the reason is now the true one: a per-report "nothing failed, nothing ran" is accurate, and it is a ONE-surface change, not two |
| 2 | **The other half of the tightened predicate was unpinned.** Relaxing `==` to `include?` survived all 2681 examples, and the state it admits is one notch out from the defect the tightening fixed: `Preflight#run` returns the configuration checks ALONE when one failed, so an adapter publishing one check under the deferral's id and one that FAILED would be reported as "nothing was verified" | measured with no doubles: `include?` → `EXIT=2, MASKED=true`; exact match → `EXIT=1` | **FIXED** with the example the reviewer supplied, validated green on HEAD and red on the mutant |
| 3 | **The retracted *"or name one above with RRD_ENGINE"* wording could be restored verbatim with the full suite green** — the only assertion on that sentence was its headline phrase, and the actionable half was unasserted, on the line the README and CHANGELOG both advertise | the mutant survived 2681 examples | **FIXED**: the two remedies are asserted, and both the revert and dropping them outright are now killed |
| 4 | **The commit's premise about `rake -T` is false on a terminal.** Rake truncates `-T` to the terminal width: measured on a real PTY at 80 columns the line ends at *"Render a probe document throug…"* — 35 characters, with the first exit-code word at offset 77. And the new control asserts the stored comment, not the rendered line, which is the same shape as a defect this feature already fixed once | `script -qec` at 80/100/120 columns | **FIXED by correcting the premise**, not by front-loading: no description that also says what the task DOES survives a 35-character cut, so the comment now says `-D`, a piped `-T` and the README are where the codes are legible, and the control says out loud that it checks the stored string |
| 5 | Minors: the `desc` named three of the four documented ways to reach 2 (the unparseable selection — which `script/render_preflight_exit_codes.sh` MEASURES as its fourth arm — was folded away, and HANDOVER omitted it too); `implementation-plan.md:182` still cited *"E-29 row 7, which is OPEN"* where E-29 has its own unrelated row 7 and E-27's is closed; and "Six examples" went stale again in the row that had just corrected "five" | counted | **ALL FIXED**, including the fourth cause in the desc, its assertion, and the HANDOVER line |

**Measured after the fixes:** rspec **2682 / 0 failures / 127 pending** with `.git` present, minitest
**923 runs / 0 failures / 0 errors / 4 skips**, nine gate scripts OK, Ruby 2.7 floor OK. **Twelve
mutations, twelve killed, zero survived** — including both of the reviewer's survivors and dropping
the remedies outright.

**E-37 · THE TENTH REVIEW COULD NOT BREAK THE BEHAVIOUR, AND REJECTED THE COMMIT FOR THREE STALE
CLAIMS ABOUT IT.** 2026-08-12, `e0b7e51` reviewed and corrected in the commit after it. First round
of the seven in which a reviewer could add no survivor to the author's own mutation set.

**What it could not fault**, and this is the part worth keeping: it drove **18 states** of
`PreflightSuite#reports` in-process and found **no install that now exits 2 and should not**. The
shipped configuration stays 0, both `RRD_ENGINE` paths and the selected-engine path stay about the
checks, a real check that FAILED beside a deferral stays 1 rather than being masked as 2, an
unreadable catalogue defers nothing (so nothing changes), the JSON artefact parses at exit 2 off a
real process, all four arms of `script/render_preflight_exit_codes.sh` name an engine and so cannot
reach the new branch, and no CI job runs the rake task. 13 mutations, 13 killed.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **`report_run.rb` still said `PreflightCommand` "answers exit 0" and that row 7 is "OPEN and awaiting a curator decision"** — three false clauses in six lines, in the paragraph an earlier review created specifically to record that disagreement, one line above a passing test of the opposite | measured off a real process: exit 2, both surfaces agree, row 7 closed by the same commit | **FIXED.** A comment that outlives the decision it describes is how a closed question gets re-opened by the next reader. The E-29/E-27 row-7 citation mismatch is reconciled too |
| 2 | **The README, the CHANGELOG and the code comment all said the *"OK so far"* print was gone.** It is not: `Report#headline` is untouched, so it still prints per engine, one line above the new sentence | the terminal output | **FIXED in all three**, to say what actually changed — the per-engine line is that report's own summary; the run-level verdict is the exit code and the last line. **The correction itself then over-reached** — it said the line is "shared with the admin page" and that rewording it would be a change "on two surfaces", and the next review measured both false: `headline`'s only caller is `to_text`, whose only production caller is `#emit`, and the page prints its own sentence from `ReporterPreflightHelper#reporter_preflight_summary`. Rewording it would be a ONE-surface change and is still not done, for the reason that stands: a per-report "nothing failed, nothing ran" is accurate |
| 3 | **`rake -T` — the definition an operator reads first — still said 2 means "no engine is registered"**, and its test could not notice: the assertion was `assert_match(/exit/i, description)`, which a description reading *"exit 7 always, and never 2"* satisfies (measured SURVIVED) | the mutant | **FIXED**: the `desc` names all three causes and the test asserts each one |
| 4 | **A `gotenberg_spec` example was named "…and rake still exits 0"** — false in the registry its own `around` block builds, since that block leaves `:gotenberg` alone. The body never called `PreflightCommand`, so the suite stayed green while `--format documentation` printed the falsehood | read | **FIXED**, and it gained the assertion: this is the only place the REAL adapter sits alone in the registry, so the new exit code now has one measurement against the shipped class rather than only against a stand-in |
| 5 | **The predicate keyed on an id any adapter may publish.** `Registry` is open and `Preflight#configuration_checks` copies `entry[:id]` verbatim, so a third-party adapter whose check FAILED under that id was relabelled "nothing was verified" | measured as state 18; non-zero either way, so no deploy step was misled | **FIXED** by matching `[id, state]` — only a `:skip` can satisfy it |
| 6 | **The plan's own closure recorded five examples where the diff adds six**, and the omitted one is the shipped-configuration guard — the proof that matters most, missing from the record of what was proven | counted | **FIXED** |
| 7 | Minors: HANDOVER's exit-code line was incomplete in the file §2.0 orders read first; "or name one **above** with RRD_ENGINE" pointed at nothing; the `zh` occurrence count was five where it is six others; and the entry sat under `### Added` for a change that can turn a deploy step red | counted | **ALL FIXED** |

**Recorded, NOT acted on, both deliberately:**

* **The admin preflight page has no equivalent statement.** `ReporterPreflightController` bypasses
  `PreflightCommand`, so on the same all-deferred install the page shows a green tick per engine and
  says nothing about the run having verified nothing. Same reachability as the rake case
  (unreachable on the shipped tree), and the fix is a view change on a second surface — the
  curator's accepted scope was "one condition in `#call` and one README bullet". **This is the next
  thing to do if the state ever becomes reachable**, and it is the third time this rule has needed
  writing in a new place (E-27's own observation).
* **`pl` now says more than the English source** — *"działania osobnej usługi renderowania"* against
  en's "a service". Grammatical, and the vocabulary is the file's own, but a future English rewording
  will not be visible against it. Deliberate, per §Findings E-36.

**E-36 · A SECOND MODEL WAS ASKED TO REVIEW THE NINE TRANSLATIONS, AND THE USEFUL ANSWER CAME FROM
THE MECHANICAL QUESTION, NOT THE POLITE ONE.** 2026-08-12, at the curator's instruction and with
their credential. Worth recording as method, because the first framing would have shipped a
regression.

Three differently-framed requests about the SAME `hu` string, to the same model, gave three
different verdicts:

| framing | verdict on the shipped `hu` value |
|---|---|
| *"review this string"* | **ok** — no change needed |
| *"choose between these candidates"* | **ungrammatical**, and it recommended a form an earlier review had rejected for subject agreement |
| *"is each grammatical; what is the subject of the second clause; what does it assert"* | **grammatical, but asserts something else** — the second clause's subject reads as *"a telepítés által kiválasztott motor"*, so the sentence stops saying what it must |

Only the third is actionable, and it is the one that found the real defect: the `hu` value shipped
in `9d99a37`/`00b6fc7` was a REGRESSION introduced by fixing the previous one. The form now
shipping satisfies both reviews at once — the engine is clause 1's subject (the earlier finding) and
`ez` is clause 2's (this one). **The lesson is the shape of the question:** "review this" invites a
suggestion whether or not one is warranted, and a preference is indistinguishable from a defect in
its answer. Ask what the subject is and what the sentence asserts, and the answer can be checked.

**What changed:** `hu` back to the form judged grammatical AND correct in meaning; `pl` gains the
explicit *"działania osobnej usługi renderowania"*, which two independent framings both asked for
and which the file's own vocabulary already contains. **What deliberately did not:** `zh`. A native
pass prefers `本安装` over the `此安装` this work standardised on — but `此安装` is what the other six
occurrences in that file use (seven in total; the first version of this sentence said five), and §10's rule is one vocabulary per locale file. Changing it is a
six-string question for the curator, not a one-string divergence. Recorded rather than acted on.

**What this does NOT settle:** these are one model's judgements, obtained three ways, and the
disagreement between the framings is itself the evidence for treating them as a review rather than
as truth. `hu`, `pl` and `zh` still want a human native reader; what changed is that a wrong value
now fails a test instead of shipping.

**E-35 · THE NINTH REVIEW: A §6 FIX THAT FIXED NOTHING, AND A GUARD DELETED ON A PROOF THAT DID NOT
COVER THE CASE IT WAS FOR.** 2026-08-12, `9d99a37` reviewed and corrected in the commit after it.
Sixth consecutive rejection; the behaviour was right and two claims about the change's own guards
were not.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **`I18n.with_locale('en')` was decorative, so the §6 violation was unchanged — and E-34 row 6 recorded it FIXED.** `ApplicationController#set_localization` is a `before_action` doing `set_language_if_valid(find_language(user.language))`, so it runs INSIDE the block and overwrites it | measured both directions: `with_locale('de')` still rendered ENGLISH and still PASSED (the mutant survived), and `@admin.language = 'de'` rendered GERMAN and FAILED with `with_locale('en')` still in place. The locale is `User#language`, whose value came from Redmine's own branch-versioned `test/fixtures/users.yml` — the inheritance §6 forbids | **FIXED** by setting `@admin.update_column(:language, 'en')`, which is where the controller reads it. Re-measured: setting it to `de` now KILLS the test, so the line governs the outcome instead of decorating it |
| 2 | **The two deleted `not_to match` guards could be the deciding assertion**, and the deletion's proof only covered mutation of the production string | measured the class this project has actually been bitten by three times — reword the message to name a cause AND update the `eq(...)` with it, as an author does: **SURVIVED at `9d99a37`**, and KILLED with the deleted lines restored | **`/\bbecause\b/` RESTORED** and re-measured killed. `/\bpresent\b/` stays deleted: no draft ever smuggled that word in. The comment now states both measurements — equality subsumes code-only mutation, the keyword guard covers reword-with-expectation — instead of claiming the guard could never decide |
| 3 | **The hand-written nine-locale list this commit's own comment condemns sits sixty lines above it, unchanged** | a tenth locale file carrying ONE key passed `test_every_key_the_page_uses_exists_in_every_locale`, the test whose whole promise is the opposite | **FIXED**: one derived `SHIPPED_LOCALES` constant, used by both tests |
| 4 | **A quoted log line that does not ship**: the comment wrote `:from_settings` where `warn_line` interpolates `id.to_s.inspect`, so the line reads `"from_settings"` | measured under the mutant | **FIXED** — the same class E-32 row 5 and E-33 row 8 rejected commits for |
| 5 | **"the same call this change already made in `report_run_spec.rb`" attributed a deletion to the wrong commit** — it was `df2144b`'s | `git show 9d99a37 -- spec/reporting/report_run_spec.rb` deletes no assertion | **FIXED**, and it matters: the deletion was propagating an earlier decision, not reconciling with one taken alongside it |
| 6 | Minors: the equality loop aborted at the first wrong locale (an eight-locale revert reported only `de`); `zh` gained a second term for "this installation" (`本安装` ×2, both introduced by this work, against `此安装` ×5 pre-existing) breaking §10's one-vocabulary rule; the log guard passed on an EMPTY log; and `hu`'s negation on `ez` reads contrastively | measured/counted | **ALL FIXED**: the loop collects and asserts once, `zh` uses `此安装` throughout, the clean-state expectation is `eq([])` positively, and `hu` is *"nem a telepítés által kiválasztott motor"* |

**Measured after the fixes:** rspec **2674 / 0 failures / 127 pending** with `.git` present, minitest
**923 runs / 4267 assertions / 0 failures / 0 errors / 4 skips**, nine gate scripts OK, Ruby 2.7 floor
OK. Both blocker mutants — the locale set and the reworded message — are KILLED.
...
| 6 | Minors: the "Recorded, not fixed" paragraph measured the 90-character cut of the message THIS commit replaced (`…at the c` versus the shipped `…red path was r`) — the same stale-quote class the row above it claims to have fixed; a truncated quote presented as verbatim in `report_run.rb`; an unreflowed comment orphan; and the new functional test inherits its locale from a core fixture rather than setting it (§6) | re-derived | **FIXED — except the last one, which this row claimed and did not deliver.** The `I18n.with_locale` wrapper it added is decorative: Redmine's `set_localization` `before_action` resets the locale from `User#language` inside the block. Measured and actually fixed in §Findings **E-35** row 1 |
...
right and a CLAIM was not.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **The admin preflight page does not print `Check#title`.** E-32 row 3 corrected the title and said that fixed "the admin page and the rake task". The page prints a LOCALE LABEL, and all nine locale files still said *"and this install has not chosen it"* | five sources, one of them the helper's own header (*"The title is for the rake task and the JSON artefact… the ADMIN PAGE is a user-facing view, so §10 applies"*). Measured on a rendered page: the `<td>` an admin reads carried the falsehood while the corrected sentence appeared only inside the collapsed JSON. And nothing could catch it — two functional tests assert the KEY exists, one of them in all nine locales, neither asserts a word of its content | **FIXED in all nine locale files**, each in its own language, and pinned by a functional assertion on the rendered `<td>` plus a `translation missing` guard. The title keeps its own (already correct) wording for the rake and JSON surfaces. **The hu, pl and zh wordings are mine and want a native reader** |
| 2 | **A paragraph three lines above the new one still asserted what two measurements had killed:** *"`EACCES` means the file IS there and is not executable — one fault, one remedy"* | same file, same commit, unchanged because the diff that killed the sentence never looked at the comment beside it | **FIXED**, and the same slip existed in the spec's comment and in two plan rows, all corrected |
| 3 | **Two of the round's new guards passed a false sentence.** The EACCES guard (three keyword matchers) passed *"…refused, because the filesystem holding it is mounted noexec"* — a THIRD confident enumeration; the title guard passed the title INVERTED to *"and it IS this installation's selected engine"* | `36 examples, 0 failures` and `164 examples, 0 failures` | **FIXED by pinning both sentences with equality.** A message this project has been wrong about three times is not guarded by keywords. A third guard fell the same way — the misconfigured message reworded to *"you have selected nothing"* — and is pinned the same way |
| 4 | **The replacement discriminator is unreachable in production too**, one layer over from E-32 row 1's defect: `EnginePreference` drops the stale id and hands `ReportRun` nil, so `selected_engine_id`'s `warn_line` never runs in a booted Redmine. The line an operator gets is `EnginePreference`'s own | measured: `selected_id = nil`, `dropped? = true`, and the dropped-value log line | **FIXED in the comment**, which now names both lines and which surface each reaches |
| 5 | **`EACCES` has one cause this side CAN establish and the checklist missed it**: a path naming a DIRECTORY. All three things the message says to check come back clean | measured `Open3` against a directory → `EACCES` | **FIXED with a branch**, not a longer list: `File.directory?` settles it, and the project's rule is to discriminate where you can and name more than one remedy where you cannot. `File.directory?` is false for a path that cannot be stat'ed, so the branch only claims what it saw |
| 6 | **`_diagnostics.html.erb` and `report_run.rb` shipped contradicting claims about the same field** — *"It is in the application log"* versus *"no logger writes it"* | re-derived: nothing writes `Diagnostic#detail` anywhere | **FIXED in the ERB**, which was the false one. A round about false claims had corrected the other file and left this |
| 7 | **The deletion cost rspec its only `FROM_SETTINGS` coverage.** Ignoring the sentinel entirely survives the whole rspec suite; only a Minitest functional test kills it | `2672 examples, 0 failures, 127 pending` under the mutant | **FIXED**: a minimal example covers the branch — the port is consulted exactly once and the sentinel is never used as an engine id — without re-asserting the memo the round deleted. CI was never at risk; a branch covered on one surface only is one deletion from being covered nowhere |
...
**E-32 · THE SIXTH REVIEW REJECTED THE CORRECTIONS TO THE CORRECTIONS, AND THE FIX FOR THE THIRD
STATE HAD BEEN WRITTEN INTO A FIELD NOBODY READS.** 2026-08-11, `cdb272e` reviewed and corrected in
the commit after it. This round is mostly a DELETION, which is the right shape: the previous round
built a mechanism to carry information to a reader who does not exist.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **`Diagnostic#detail` is printed NOWHERE, so `selected=<value>` reached nobody — and three new comments called it "admin-facing".** The memoisation of `engine_preference` existed only to serve it | four of this plugin's own sources say so: `_diagnostics.html.erb` lists `Diagnostic#detail` under *"WHAT IS DELIBERATELY NOT PRINTED"*, `Diagnostic#to_h` omits it *"so no future serialiser picks it up by accident"*, `FailureDocument` excludes it, and `rg '\.detail' app lib` finds only the two delivery `restamp` sites. No logger writes it | **DELETED — the detail interpolation, the memo, and the memo's example.** The information was never missing: `selected_engine_id` already logs *"this installation selects render engine "athena", which is not registered here"*, an example pins that line, and the third state's real fix was the SENTENCE being true. The spec now asserts the LOG rather than the unread field — and note WHICH log: in production `EnginePreference` writes the dropped-value line and hands `ReportRun` nil, so `selected_engine_id`'s own line runs only through the port (**E-33** measured a comment here citing the wrong one) |
| 2 | **The `EACCES` sentence's second enumeration was false too: `noexec`.** *"either not executable, or in a directory Redmine may not enter"* | measured on a `noexec` tmpfs: mode `0755`, every parent world-searchable, still `EACCES`. Neither named reason holds, and `chmod +x` changes nothing. `/tmp`, `/var/tmp`, `/home` and NFS are routinely `noexec` on hardened hosts; SELinux and AppArmor denials land on the same errno. The full matrix the reviewer measured also re-confirmed E-31 row 3 (a MISSING file under a `0700` parent gives `EACCES`) | **FIXED by stopping the enumeration.** *"Redmine was refused permission to execute the render engine at the configured path. Check the execute bit on the file, the search permission on the directories above it, and whether that filesystem is mounted noexec"* — the fact, then what to check. Two confident guesses were each wrong for a real case; the third version does not guess |
| 3 | **The identical false sentence was left standing on the REACHABLE surface.** `preflight_suite.rb`'s deferral title said *"this install has not chosen it"* — nil `selected_engine_id` for a stale stored selection, exactly the state row 5 of E-31 fixed elsewhere | the admin preflight page and the rake task both fill that port from `render_engine_id`, which drops an unregistered id. And E-31 row 8 records that both arms of `no_engine_diagnostic` are unreachable in a booted Redmine — so the fix had been applied to the unreachable copy and not the reachable one. **Nothing asserted that title at all** | **FIXED**, and now pinned: *"the render engine needs a service and it is not this installation's selected engine"*, true in all three states, with an example. The stored value is deliberately NOT threaded into a new port to carry it — the log already names it |
| 4 | **Two more of my own new guards had surviving mutants**, both on the field row 1 deleted: `detail` printing the raw ivar (`selected=from_settings` on the only production path) and a process-wide `@@` memo — the very boundary the comment argued | full suite `2673, 0 failures, 127 pending` under both | **DISSOLVED by row 1.** Third round running in which the author's own mutation set missed a guard the reviewer's found; the standing lesson is in E-31's own words, and the deletion is what finally removes the surface |
| 5 | Counts and claims, again: the plan still said `spec/golden` is "71 examples" (**167**) — in the very section whose subject was stale counts; the G7 entry compared `2670/137` with `2671/127` and framed the EXAMPLE totals as differing, when they are equal and only pending differs by ten; `plan:88` said "six sites" where eight now emit the code; *"two of CI's four cells run 3.3+"* is **three** (3.2, 3.3, 3.4, 3.4); the userinfo comment's *"most of the failure messages"* is 10 of 36 by that reviewer's rule (a later review could not reproduce that count, or the earlier one, under any rule it tried — which is the argument for the clause naming no quantity at all); that same comment claimed the failure mail goes *"to every recipient"* when the mailer sends it to the schedule's OWNER and the view says so; and HANDOVER said "the eight gate scripts" where nine run without arguments | each counted or measured | **ALL FIXED.** The endpoint clause now names no quantity at all: a number nobody updates is worse than none, and a superlative nobody counts is worse than both |
| 6 | Smaller: `.to_s.empty?` where the neighbour uses `.strip.empty?`; an unbounded port value interpolated into a diagnostic; `not_to include('present')` matching "represents"; the menu path spelled `>` in one file and `→` in another; a dead restore branch in the memo example; and the `MinimalPdf` rationale stated as *"is replaced"* when `message` cannot reach that artefact today | measured | **FIXED or DISSOLVED**: the first three go with row 1's deletion and row 2's rewording, the guard is now `/\bpresent\b/`, both files say `>`, and the rationale reads *"would be, if it ever travelled"* |

**Measured after the fixes:** rspec **2672 / 0 failures / 127 pending** in a directory with `.git`
— one example fewer than `cdb272e`, the deleted memo example, which is the point of the round.
(This paragraph first said 2670 and "three examples fewer", written before the run rather than from
it. Caught by running it. That is the whole discipline in one line: the number goes in after the
measurement, never before.) minitest **920 runs / 0 failures / 0 errors / 4 skips**, nine gate
scripts OK, Ruby 2.7 floor OK, and a mutation pass over what this round changed.

**E-31 · THE FIFTH REVIEW REJECTED THE CORRECTIONS, AND THREE OF ITS FOUR BLOCKERS WERE CLAIMS
THIS PROJECT HAD JUST WRITTEN ABOUT ITSELF.** 2026-08-11, `63f080e` reviewed and corrected in the
commit after it. Every row below was measured by the reviewer and re-measured here.

| # | What it found | Evidence | State |
|---|---|---|---|
| 1 | **A survivor my own 14-mutant set had no mutant for.** The empty-registry arm's `detail` — `'Render::Registry.ids is empty'`, the string E-30 row 2 quotes verbatim — is asserted by nothing. So "both mutants are now killed" was half true: the message is pinned, the detail was not | mutated to nonsense: targeted `47 examples, 0 failures`, **full suite `2671 examples, 0 failures, 127 pending`** — SURVIVED. `rg 'Registry.ids is empty' spec/ test/` → nothing | **FIXED**, and the claim is corrected in E-30 row 2 and here rather than left standing. The lesson is about method, not about one string: a self-written mutation set attacks what its author thought of |
| 2 | **"the failure PDF prints `message`" is false, and this plugin's own source says so.** Three new sites asserted it | `failure_document.rb` states the opposite in as many words — *"`Diagnostic#message` is NOT among them, and neither is `#detail`. The document is the artefact that TRAVELS, so it gets the narrower set"* — and the reviewer built the real object: the message is absent from the PDF bytes, **and `MinimalPdf.encodable?` is false for it**, because the sentence contained `→` and that document is Windows-1252 | **FIXED** in all three, and the `→` is now `>`. A sentence that might ever reach the travelling artefact has to stay drawable, or `FailureDocument` replaces the whole remedy with "value unavailable" |
| 3 | **The "PRESENT IS ESTABLISHED AND NOT ASSUMED" paragraph — written in the previous commit to answer this same question — is false.** The reasoning was that `execve` answers `ENOENT` for a path holding nothing, so `EACCES` proves presence | the reviewer ran a child dropped to uid 65534 against a `0700` parent: `EACCES` for a file **that does not exist**. So *"the render engine is present and cannot be executed"* was a confident wrong diagnosis of `RRD_WKHTMLTOPDF_BINARY=/opt/private/wkhtmltopdf` under a `0700` directory — an ordinary operator mistake | **FIXED, then fixed twice more.** That sentence named two reasons, a `noexec` mount refuted both (**E-32** row 2), the replacement named `noexec` as THE cause and a mutation proved the guard allowed it (**E-33**). The shipped sentence names NO cause — *"permission to execute … was refused — check its execute bit, the directories above it, and whether that filesystem is mounted noexec"* — with one branch for a path naming a DIRECTORY, the single cause `File.directory?` can establish. The CODE is unaffected: `chmod` is the remedy either way and no retry helps, which is why `:engine_misconfigured` survives a finding that killed the sentence |
| 4 | **`expect(Registry.ids).to eq([])` is a control that cannot fail, and its comment claimed a purpose it cannot serve** ("stops the two examples silently becoming one case") | the reviewer simulated a total registry leak: 8–9 examples died, and **this guard fired at 0 of 12 seeds**. Structural: RSpec runs a group's own examples before its child groups, and no adapter file is required in this DB-less process | **DELETED**, and the line spent on row 1's detail assertion, which can fail |
| 5 | **The two-state split was not exhaustive: a third state reaches the misconfigured sentence and is lied to.** A stored selection naming an unregistered engine leaves `selected_engine_id` nil, so *"and none has been selected"* was told to an operator who had selected one | measured through the public `engine_preference:` port. **Found twice independently** — an adversarial pass here probed it before the review reported it. Production cannot reach it (`EnginePreference` drops the value at the settings boundary), but the port is public, which is the same reason the empty-selection example exists | **FIXED.** The message now says only what is true in every state that reaches it — *"no render engine on this installation can be selected automatically"*. (This row also said `detail` carries the selection; **E-32** row 1 deleted that, because `Diagnostic#detail` is printed nowhere — and the whole sentence is now pinned by equality, because **E-33** reworded the lie past a phrase-matching guard.) `engine_preference` was briefly memoised per run for a second caller this round added and the next round removed (E-32 row 1) — the duplicate-read defect it described never shipped |
| 6 | **The DNS sentence's third remedy was not a remedy.** *"the resolver itself may be temporarily unreachable, and then this clears on its own"* | the reviewer's own EAI_AGAIN reproduction is a **permanent** unreachable resolver; it never clears | **FIXED**: *"if that is temporary it clears on its own, and if it is not, check Redmine's own DNS configuration"* |
| 7 | Minors: `plan:88` still said "twelve-row" and "three argued non-movers" (13 and four); HANDOVER said `rspec spec/golden` is "71 examples, 0.2 s" (**167, 0.55 s** — I had quoted a file subset as the directory); `gotenberg.rb`'s userinfo comment said `@endpoint` is interpolated into "six" failure messages (nine before this work, ten after); the new Ruby-conditional skip makes the pending total **127 on Ruby ≥3.3 and 128 on the Redmine 5.1 / Ruby 3.2 cell**, which the HANDOVER entry recorded as one number; and new prose called the settings screen "FR-50", which E-29 row 8 records as the wrong number pending a curator decision | counted and measured | **ALL FIXED.** The endpoint count is now stated without a number, because a count nobody updates in a security-bearing comment is worse than none |
| 8 | **Both arms of `no_engine_diagnostic` are unreachable in a booted Redmine**, since all three adapters register at boot and `chromium_cdp` is `default: true, needs_service: false` | `redmine_reporter_dashboards.rb:60-62` | **RECORDED, not changed.** It is pre-existing and it qualifies the severity of rows 1 and 5: the false sentences were measured through a spec harness, not in an install. The arms are still worth being true — a plugin that registers no engine is what `Registry.isolated` and any future packaging change produce |

**And the corrections were mutation-tested in turn: 13 mutants, and the first pass found 2 of my own
new guards unpinned.** Reverting the per-run memoisation of `engine_preference` survived (nothing
counted the reads — and the reason nothing could is worth recording: `render_engine_id` lives in the
boot file, which the DB-less suite does not load, so the `FROM_SETTINGS` branch raised `NameError`
and no example had ever gone near it); and restoring the DNS sentence's "clears on its own" survived,
because `/spelling/` and `/resolver/` both still matched the old wording. Both are pinned now — a
read counter and a `/DNS configuration/` matcher — and the second pass is **13 killed, 0 survived**,
control green first. That is the same lesson as row 1, one round later: the mutant you do not write
is the guard you do not have.

**What the reviewer could not verify, and who did:** minitest (needs a mirror write into the shared
tree it was told not to touch), G9, G11, G12 and the conformance corpus. G9 and minitest were
measured in the parent session for this round and are recorded in HANDOVER §4; G11 and G12 remain
UNVERIFIED and neither is reached by this change.
...
| 7 | ~~**`PreflightCommand` exits 0 when EVERY engine is deferred**, contradicting its own documented `NOTHING_TO_RUN = 2`.~~ **CLOSED 2026-08-12 — the curator's instruction was "correct the wrong one", and this was the wrong one.** Implemented as recommended, with the condition kept: every report is still emitted, so the deferral rows and their `RRD_ENGINE=<id>` remediation survive the non-zero exit. The sentence is TEXT-ONLY because `emit` may already have written the JSON artefact and a line after `JSON.pretty_generate` would stop it parsing — an example asserts the artefact still parses on the exit-2 path. **Seven** examples (six in the commit that made the change, one added by the round after it — and the first version of this row said five, which is the same arithmetic going stale twice): all-deferred exits 2, the rows survive, the JSON parses, the two ways of asking (an installation that SELECTED the engine, and `RRD_ENGINE=<id>`) both go back to being about the checks — the second pair is what stops this making every Gotenberg-only install permanently red, which is what FR-50 bought — **and one engine checked beside one deferred stays 0, which is the shipped configuration and the one that must not go red.** That last one is the example a mutation found missing (`all?` relaxed to `any?` survived the whole suite), and the first version of this row counted five and left it out — the proof that matters most, omitted from the record of what was proven. README's exit-code bullet changed in the same commit, since it is a documented interface. The original finding and its recommendation | **RE-MEASURED 2026-08-11**, not read: `Registry.isolated { register(:gotenberg) }` then `PreflightCommand.new.call` → **exit 0**, and the sentence a human reads is `render preflight: gotenberg not checked (OK so far — 1 check(s) could not run, 0ms)`. So a deploy step is told "fine" and a person is told "OK so far" about a run that verified nothing. Unreachable in the shipped 3-engine tree (two engines need no service), and reachable the moment a tree has only service-backed engines | **Recommend YES, with one condition and one consequence.** *Why yes:* the code's documented meaning is "nothing was verified", that is exactly what happened, and this repository's oldest defect class is a green run that checked nothing (INV-7). The cost is one condition in `#call` and one README bullet. *The condition:* keep the DEFERRED REPORTS in the output. Exit 2 with a blank page would replace one bad answer with another; the skip rows and their `RRD_ENGINE=<id>` remediation are the only actionable thing on that surface. *The consequence to accept:* 2 is non-zero, so an install whose only engines need a service goes from a green deploy step to a red one — which is correct (nothing was verified) and is a behaviour change on a documented interface, hence the curator's call. *And the reason the urgency is lower than it looks:* **FR-50 removes the realistic instance of this**. Once an install can SELECT an engine, the selected one is in the preflight's default set even when it needs a service — selecting it is the decision the deferral was waiting for — so the practical "the engine this install actually renders with was never checked" case is closed by that change rather than by this one. What remains for row 7 is the honest-exit-code argument on a tree nobody ships yet |
...
**DONE 2026-08-13 — see §Status. Three things a later reader should not re-derive.** The drop
reference is `lib/…/liquid/drop_reference.rb` and NOT `render/drop_reference.rb` as the
`Touches:` line below says: `layer_purity.sh`'s render arm forbids `render/**` from naming
`Liquid` at all, so a file there that reads a drop class is a G8 failure on its first line. The
gallery's five entries are PURPOSE-BUILT under `starters/` rather than the two legacy examples
"cleaned" — those carry 12 and 44 lint findings, every one of them the hand-built Chart.js 2
plumbing `{% chart %}` replaced, so modernising them is a rewrite of an 800-line dashboard and
the result would still be an 800-line dashboard. And the chart form has no `drill:` control,
because the chart tag has no such parameter — drill-through is `{% sql_aggregate … drill: true %}`
and the form says where the switch is instead of writing one the tag ignores.
...
**DONE 2026-08-13 — see §Status. Two things a later reader should not re-derive.** The stylesheet is
RUBY and not a `.css` file, because the palette clause ("read from the chart layer, not duplicated in
CSS") is unsatisfiable by a file that cannot read a constant. And the two capabilities this task
briefly added to the closed vocabulary — `:repeating_table_header`, `:svg_link_annotations` — were
**retracted**: both were measured against `apt install wkhtmltopdf`, which is the UNPATCHED-Qt build,
and the patched build CI uses does both. There is no support difference to declare.
...
### Phase 6 — which project a report is about *(ships 0.9.0; curator 2026-09-18, cut down 2026-09-20)*

**TWO PLANS WERE KILLED BEFORE THIS ONE.** Both tried to bind the saved query to the surface's
project the way `QueriesHelper#retrieve_query` does. A red team and an independent review each built
that and measured it. It works for the row count and it breaks something else, and the third version
is deliberately much smaller than the first two.

#### The two defects, measured three times independently

Fixture: project 1's descendants are 3, 4, 5, 6. **Project 2 is a sibling, not family.** The recipe
is `docs/plan/reference/verification-project-scope.md`.

| | setting ON (default) | OFF |
|---|---|---|
| Redmine's own issue list, global query, in project 1 | 10 · `[1,3,5]` | 4 · `[1]` |
| plugin project surface (dashboard, page, PDF, mail) | 10 · `[1,3,5]` | **10 · `[1,3,5]`** |
| plugin `/my/page` | **11 · `[1,2,3,5]`** | **11 · `[1,2,3,5]`** |

**D-1.** On a project surface the plugin ignores `Setting.display_subprojects_issues?`. With the
setting on, which is the default, it already answers what Redmine answers; with it off it does not.

**D-2.** On `/my/page` a project-less query is unbounded, sibling project 2 included, and nothing can
narrow it. This is the curator's original observation.

#### Why the bind-the-query approach was abandoned — MEASURED, and it reverses the argument that sold it

Every figure in a report is a link to the issue list that produced it. Measured end to end with a
global query carrying its own `project_id` filter, on project 1's surface:

```
today                     figure 3  ->  list 3    they agree
with the query bound      figure 3  ->  list 10   they do not
```

`IssueQuery#initialize_available_filters` offers `project_id` **only when the query has no project**.
Binding removes it from the drill-through query's available filters, the URL still carries it, and the
receiving list discards it. So the approach breaks the very property it was recommended for. A
proposed follow-up task to repair that was built and measured too: it did not repair it, and the
refusal it needed is not expressible where it would have to live.

**So this phase does not touch the query object at all.** No assignment, no `readonly!`, no ordering
requirement, no frozen kernel file, no new refusal. What is left is the two defects above.

#### Deliberately unchanged

* **The no-query path keeps its exact-project bound.** Widening it was the curator's instruction of
  2026-09-18 and my recommendation, and both were wrong: a red team measured an actor whose role lacks
  `:view_time_entries` in project 1 seeing `COUNT=1` from project 3, where they are a non-member,
  under a notice still saying their role shows them no hours here. §Findings **S-14** rebuilt.
  DECISIONS-PENDING #19.
* **Drill-through agreement** is its own question, mis-diagnosed in the previous version of this text
  and now recorded as DECISIONS-PENDING #21 rather than carried as a task.

**On a default installation this phase changes no existing figure.**

---

**T-51 · THE PROJECT BOUND FOLLOWS REDMINE'S SUBPROJECT SETTING.** *(deps: none)*

*Touches:* `lib/redmine_reporter_dashboards/reporting/report_scope.rb`,
`test/unit/reporter_dashboards_report_scope_test.rb`,
`test/functional/reporter_dashboards_templates_controller_test.rb`, `docs/user-guide.md`.

*Accept:* `project_subtree_ids` answers `[project.id]` when `Setting.display_subprojects_issues?` is
false and `[project.id] + non-archived descendants` when it is true. Nothing else in `ReportScope`
changes: `within` stays, the no-query path stays, the query object is never touched. Both sources are
covered, because `ReportScope` serves issues and time entries through one method and an administrator
switching a setting named for issues also narrows spent-time reports — that is Redmine's own
behaviour and the user guide says so.

*Tests:* the setting ON and OFF, for `IssueQuery` and for `TimeEntryQuery`, each asserting the row
count and the set of project ids; a sibling-project row that must never appear under either setting;
an archived descendant that must not appear under either.
`test_a_global_query_does_not_pull_hours_from_outside_this_project_subtree` keeps its subject and
gains the OFF case, because its present name and comment would otherwise become false.

*Invariants:* INV-1 is untouched — `ReportScope` already threads the actor and this changes only an
id list. It is worth recording where INV-1 actually rests on this path: `Query#base_scope` calls
`Issue.visible` / `TimeEntry.visible` with **no argument**, so it reads `User.current`, and what holds
the invariant for a saved query is `Snapshot#as` and `ScheduledDelivery#as` setting `User.current` to
the actor around the render. That is pre-existing, it is not made worse here, and `report_scope.rb`'s
comment should say it rather than imply the threading covers it.

*G6:* the id list is computed once per `resolve` as it is today, so nothing moves. No query-count
change is expected and `reporter_dashboards_drill_query_count_test.rb` is the place it would show.

**T-52 · A PROJECT ON THE `/my/page` WIDGET.** *(deps: T-51)*
...
`app/views/reporter_dashboards/widgets/_my_report.html.erb`,
`app/views/my/blocks/_report_by_issues.erb`, `_report_by_spent_time.erb`,
`config/locales/*.yml` (twelve), `test/unit/reporter_dashboards_widget_report_test.rb`,
`test/integration/reporter_dashboards_my_page_block_test.rb`, `docs/user-guide.md`.

*Accept:*

1. A `project_id` in the widget settings. Blank is the default and means exactly today's behaviour.
   Chosen, it is passed to `WidgetReport.run` and T-51's bound applies.
2. The picker lists projects where the actor holds `view_reporter_dashboards_reports`, ordered like
   Redmine orders projects, with a blank first option.
3. **Resolved on every read with `find_by` and a permission check, never `find`.** `find` raises for
   an unknown id and returns for a forbidden one, and the my-page partials rescue and print an error
   placeholder — which would make the pair a usable oracle for which project ids exist. Validation is
   on read rather than on write because `MyController#update_page` stores `settings.to_unsafe_hash`
   with no hook to validate on write; the settings partial's header already records that.
4. **A stored project that no longer resolves FAILS CLOSED**: the widget renders no report and says
   the project is unavailable. It must not silently fall back to "every project", which is wider than
   what the owner configured (CLAUDE.md §4).
5. **The template lookup is not narrowed.** `WidgetReport.render` passes `project:` to `template_for`,
   which would scope templates to `[nil, project.id]` and silently stop resolving an existing widget's
   template. The chosen project bounds the rows only.
6. **The spent-time notice becomes per-project when a project is chosen.** `time_entry_notice_key`
   uses `state_across_projects`, which answers `:all` if any membership grants it — with a project
   chosen that is the wrong sentence over the right figure, which is S-14's shape again. With a
   project it asks `state(actor, project)`, exactly as the project dashboard does.
...
per-project one when a project is chosen and the across-projects one when it is not.

*Invariants:* INV-3 — the unavailable state is the fail-closed answer. INV-4 — it says so rather than
...
**Not in this phase, recorded instead:** the ad-hoc mail and share-link controllers still refuse a
global template (DECISIONS-PENDING #20); `{% sql_aggregate query_id: %}` bypasses every project bound
(#22); the golden byte-identity specs skip themselves in a git worktree (#23).
