...
| 3 | **The "PRESENT IS ESTABLISHED AND NOT ASSUMED" paragraph — written in the previous commit to answer this same question — is false.** The reasoning was that `execve` answers `ENOENT` for a path holding nothing, so `EACCES` proves presence | the reviewer ran a child dropped to uid 65534 against a `0700` parent: `EACCES` for a file **that does not exist**. So *"the render engine is present and cannot be executed"* was a confident wrong diagnosis of `RRD_WKHTMLTOPDF_BINARY=/opt/private/wkhtmltopdf` under a `0700` directory — an ordinary operator mistake | **FIXED.** The sentence now names both reasons and claims neither: *"could not be executed — it is either not executable, or in a directory Redmine may not enter"*. The CODE is unaffected: `chmod` is the remedy either way and no retry helps, which is why `:engine_misconfigured` survives a finding that killed the sentence |
