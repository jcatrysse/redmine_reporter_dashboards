...
| T-31 | **increment 1 of 2 done, and its review REJECTED the first attempt** — one blocker and five majors, every one probe-backed. The blocker is §Findings **S-15**: `ScheduledDelivery` never learned the source branch, so a scheduled time-entry report rendered `Issue.visible` and mailed `COUNT=[7]` where the visible entry count was 3, `ok=true`, nobody told. `Reporting::ReportScope` is now the ONE place that answers "which rows is this report about"; both callers go through it and T-32 gets it free. The guard also existed on only one of two kernel entry points (`{% version_rollup %}` was saved by an accident of which expressions AR qualifies), the `aggregation_source_unsupported` degradation reached nothing a user could see, a saved query could pull another project's hours onto this project's page, and `TimeEntryVisibility` disagreed with `TimeEntry.visible` whenever the `time_tracking` module was off (for administrators too) or an `own` role belonged to somebody not logged in. All 13 findings fixed. **37 mutations, 37 red** — six after rewriting a vacuous example, including one whose fixture could not discriminate because project 3 turned out to be a DESCENDANT of project 1 |
