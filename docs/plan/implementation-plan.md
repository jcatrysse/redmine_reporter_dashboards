...
| T-31 | **increment 1 of 2 done** — `source` accepted end to end. `RenderContext` gained a `source` (`:issues`/`:time_entries`, closed set, refused rather than coerced) which is what stops S-13 recurring: **`{% sql_aggregate %}` now REFUSES a non-issue scope** with a recorded degradation instead of answering issue counts under time-entry labels, and the refusal reads the source off the context rather than sniffing the relation — every legacy scope is an issue scope and carries no annotation, and the tag specs' doubles answer no `model` at all. One controller, one CRUD, one preview serve both sources (`[OQ-H]` intact); the separation is at the QUERY, `IssueQuery` vs `TimeEntryQuery`. `Drops::TimeEntriesDrop` finally has a producer, six years of `Job#issue` became `Job#record`, and `ReportRun`'s `unsupported_source` refusal became a closed-set check that a stored `invoices` still trips. **S-14 is built**: the `own`/`none` narrowing is announced on the page, `Reporting::TimeEntryVisibility` names which state applies, and its agreement with `TimeEntry.visible` is asserted by computing both. 190 keys x 9 locales. **25 mutations, 25 red**, three of them after rewriting a vacuous example — including a `case`'s `else` DELETED as a second mechanism (T-25's precedent) once mutation showed both callers refuse first. **STILL OWED — increment 2, clauses 4-7:** the owned time-entry aggregator, its positional grouped reads, the Ruby-side oracle and the three-engine run |
