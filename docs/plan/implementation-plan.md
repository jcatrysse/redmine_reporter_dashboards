...
| 6 | **`:gotenberg` cannot be diagnosed from the ADMIN PAGE at all.** `ReporterPreflightController` passes no `engine_ids` and there is no engine selector, so the deferral always applies and the skip tells that reader to run something they have no shell for | the controller and the view, read | Two surfaces that "share every decision" no longer do for the one engine with configuration to check. **Recommend** an engine parameter on the preflight route — small, and it is FR-50's neighbour rather than FR-50 |
| 7 | **`PreflightCommand` exits 0 when EVERY engine is deferred**, contradicting its own documented `NOTHING_TO_RUN = 2` | measured; unreachable in the shipped 3-engine tree | **Recommend** treating an all-deferred run as `NOTHING_TO_RUN`. Not done here because it is a change to a documented exit-code contract, which is a curator call |
| 8 | **`RRD_ENGINE=''`, `'  '` and `','` silently mean "the default set"**, while a typo'd id correctly exits 2 | measured | **Recommend** treating a non-empty-but-unparseable value as the typo case |
| 9 | **A credential in the endpoint's QUERY STRING is still accepted**, unused, and interpolated into the same six messages userinfo was refused for | `http://h:3000/?token=abc` accepted | **Recommend** the same refusal. Left because the argument for refusing userinfo is that it is the natural spelling; a query token is not |
| 10 | **`/version` is fetched twice per preflight**, and configuration checks report `duration_ms: nil` so the page prints "0 ms" | measured | Cosmetic and cheap; **recommend** memoising the identity probe's body and stamping the durations |
| 11 | **No HTTPS coverage anywhere**, and CI never runs the preflight rake task | `use_ssl: false` mutation survived | **Recommend** one example pinning `use_ssl` from the scheme, and a render-smoke step that runs the rake task so the exit-code contract is exercised somewhere |
