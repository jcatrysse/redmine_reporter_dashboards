...
pass prefers `本安装` over the `此安装` this work standardised on — but `此安装` is what the other five
occurrences in that file use, and §10's rule is one vocabulary per locale file. Changing it is a
