...
**S-21 · `import:verify` is NOT built, and its `Accept:` clause needs a decision before it
can be. T-24 is otherwise complete. Owner: the curator.** The clause is *"`import:verify`
diffs **aggregation result hashes**, not HTML — the corpus discipline applied to user
data."* The intent is clear and right; what it diffs the hashes AGAINST is not, and the two
readings need different machinery:

| Reading | What it would compare | Why it is not obviously buildable |
|---|---|---|
| source vs copy | what the BASE plugin's renderer answers against what ours does | the base plugin is private, is not installed in CI, and after a migration is usually uninstalled — the comparison is unavailable exactly when it is wanted |
| before vs after | a hash recorded at import time against one computed later | buildable today, and it is the corpus discipline literally: freeze the numbers, detect drift. But it needs somewhere to record the hash, and §7's table list has no column for it |

The second reading is the one I would build, and it needs one decision — a
`verified_digest`-shaped column on `reporter_dashboards_templates` (§7 rule 6 says a column
arrives with its table, so this would be an argued migration like 008 and 009), or a
deliberate statement that the hash is recomputed and printed rather than stored.

**Not started rather than half-built.** CLAUDE.md §6 forbids leaving a half-finished
mechanism in the tree, and a `verify` task that prints a hash nobody can compare against
would be exactly that. `import:run` and `import:status` are complete, tested and
mutation-tested; this one clause is reported.
