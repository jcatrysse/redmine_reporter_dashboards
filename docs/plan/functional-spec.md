...
| FR-54 | **NARROWED by curator decision, 2026-08-09 (§Findings S-28).** A share link authorises **one document and grants nothing it points at**. An attachment referenced by a shared report is NOT made reachable to the link holder: the reference travels into the document unchanged and is subject to Redmine's own permission check when followed, so a public-link holder with no account gets a sign-in page rather than the file. *This replaces "attachment URLs are scoped to the share link that produced them, and expire and revoke with it"*, which would have required a **second bearer-token surface** with its own expiry and revocation — the exact complexity the frozen-snapshot design exists to remove. **Consequence, stated rather than buried:** a report that references an attachment by URL cannot display it in a shared document at all (see FR-63/FR-64 and §Findings F-16 — the asset resolution layer that would embed it has no caller yet). That is a REPORT defect rather than a sharing one and is owned by T-33 | INV-8 |
...
| FR-62 | Public report links are available, **off by default**, and serve a snapshot — so "public link" never means "visibility check skipped". **AMENDED by curator decision, 2026-08-09 (§Findings S-28): the switch is a ROLE PERMISSION plus a PER-LINK choice, not a per-template flag.** `publish_reporter_dashboards_reports` is granted to no role by default and the person creating each link chooses public or not; the curator considered and declined a per-template veto (*"no, permissions will do"*), on the grounds that two switches which must agree are two switches that can disagree. *This replaces "enabled per template"* | INV-1 |
