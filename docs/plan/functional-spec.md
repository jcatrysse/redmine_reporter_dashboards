...
|---|---|---|
| **Report author** | `manage_report_templates` (or its successor) | Writing a Liquid template once and having it render correctly in HTML *and* PDF. **This permission is a code-execution privilege (INV-9) and the spec must say so.** |
...
| FR-60 | Reporting over **time entries** is supported through a `source` field on the template, sharing one controller, one CRUD and one aggregation vocabulary with issue reporting — not a parallel branch | — |
...
| FR-68 | **Mermaid diagrams** are supported via a block tag. The rendered SVG is **sanitised by the plugin** against a closed element/attribute allowlist *after* the library produces it, not trusted because the library was configured. On an engine without JavaScript the diagram **source** is emitted, labelled, with a `Degradation` — never a blank space | INV-9 |
