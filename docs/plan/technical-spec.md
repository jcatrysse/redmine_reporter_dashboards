...
| `mail_reporter_dashboards_reports` | R | — | `loggedin` | Send a report by e-mail on demand, to Redmine users. It maps `#index` as well as the sending actions — an audit nobody may open is not an audit — and mapping it is **not** sufficient: `MailController` additionally requires `view_…_reports`, because mailing a report is a way of reading it | **live** |
| `share_reporter_dashboards_reports` | R | — | `member` | Create a share link: an expiring, revocable URL serving a snapshot to whoever holds it. It maps `#index` as well as the writing actions — a list of what you have shared that you may not open is not a control — and mapping it is **not** sufficient: `ShareLinksController` additionally requires the template to be VISIBLE to the actor, because you cannot share a report you cannot open | **live** |
| `publish_reporter_dashboards_reports` | R | — | `member` | Turn a share link into a **public** link, reachable without a Redmine account (FR-62). It maps the same two actions as `share_…` because the public/private choice is a property of the REQUEST BODY rather than of a route; the controller checks it where the field is read, and REFUSES rather than silently downgrading — somebody who ticked "public" and got private would hand the link out believing it was public. Holding this one ALONE reaches nothing | **live** |
...
| ~~**OQ-E**~~ | **CLOSED 2026-08-12 by measurement: Redmine 7.0 ships Propshaft.** `redmine/Gemfile.lock` → `propshaft (1.3.2)`, and `application_helper.rb`'s own overrides of `javascript_include_tag`/`stylesheet_link_tag`/`image_tag` carry the comment *"option keys is converted to a string in Propshaft::Helper"*. So the §6 sentence *"then neither pipeline is involved"* is **wrong for 7.0** — Redmine's override calls `super`, which IS Propshaft. **The design genuinely does not depend on it, and that was verified rather than assumed**: the PDF path reads the plugin's OWN `assets/` directory (`assets/bundled_assets.rb` — `PLUGIN_ROOT/assets`, mapped to the `/plugin_assets/<name>` URL prefix), never `public/plugin_assets/`, so a digested URL cannot cost it a stable on-disk path. The HTML path goes through Redmine's own override and works on 7.0 — the full-application suite is green there. **Still open, narrowly:** whether Propshaft digests the `plugin_assets/…` URL it is handed, which is cosmetic for the HTML path and load-bearing for nothing measured so far | §6 |
...
| ~~**OQ-M**~~ | **CLOSED 2026-08-12 BY MEASUREMENT: no — and it fails before Redmine is involved, so the "on Redmine 7.0" half of the question never had to be reached.** There is no pre-built CodeMirror 6 bundle to vendor. `codemirror@6.0.2` is a **meta-package**: `type: module`, `exports` carrying only `import`/`require`, no UMD, no IIFE, no `browser` entry, and `dist/index.js` is a **4,737-byte re-export shim** importing **7 bare specifiers** (`@codemirror/view`, `/state`, `/language`, `/commands`, `/search`, `/autocomplete`, `/lint`), each with its own transitive ESM dependencies. A browser cannot resolve a bare specifier without an import map, so getting one usable file means running a bundler — the build step §6 forbids — and the artefact would then be **ours**, which the three-independent-origin byte-identity check every other vendored asset carries in `THIRD_PARTY.md` cannot apply to. Contrast the two assets that *are* vendored: `mermaid.min.js` (upstream esbuild IIFE assigning `globalThis["mermaid"]`) and `chart.umd.js` are single files published by their own authors. **Curator decision, same day: take the fallback this section already names** — a plain `<textarea>` plus the server-rendered lint panel, which satisfies FR-71. Consequence for T-37: its `Accept:` clause promising findings "in the gutter" is not deliverable and must be reworded rather than quietly dropped | §9b.1 |
