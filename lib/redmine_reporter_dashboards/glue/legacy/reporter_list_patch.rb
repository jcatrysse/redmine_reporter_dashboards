...
# the duration of the render. Legacy ScopeResolution#resolve_query reads it LAST, after
...
module RedmineReporterDashboards
  module Glue
    module Legacy
      # MOVED HERE BY T-07. It owns the thread-local, and
      # script/gates/no_thread_local.sh now asserts that `Thread.current` appears
      # nowhere in lib/ or app/ EXCEPT under glue/legacy/ — so this file is where the
      # construct is allowed to live, and moving it is what makes that gate mean
      # something. The owned path (Liquid::ScopeBinding) carries the query in a
      # RenderContext instead, which is an argument rather than ambient state.
      module ReporterListPatch
        ...
        def rrd_with_issue_query(query)
          key      = RedmineReporterDashboards::Glue::Legacy::ScopeResolution::QUERY_THREAD_KEY
