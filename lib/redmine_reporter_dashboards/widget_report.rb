    ...
    class << self
      ...
      def templates_for(project:, actor:, source:)
        Template.visible(actor)
                .where(project_id: [nil, project&.id], source: source.to_s)
                .order(:name)
      end
      ...
      #
      # Returns `ReportRun`'s outcome untouched: it already carries `sections`, the
      # truncation counts FR-58 wants shown, the degradations, and the engine stamp, and
      # re-wrapping it here would be a second vocabulary for the same thing.
      def render(project:, actor:, block:, settings:, logger: Rails.logger)
        ...
        run(template: template, actor: actor, project: project,
            query_id: settings_value(settings, :query_id), logger: logger)
      end
